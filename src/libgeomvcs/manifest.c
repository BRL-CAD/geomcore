...
*/
void manifest_destroy(struct vcs_db *db, Manifest *p){
  if( p ){
    blob_reset(db, &p->content);
    free(p->aFile);
    free(p->azParent);
    free(p->azCChild);
    free(p->aTag);
    free(p->aField);
    free(p->aCherrypick);
    if( p->pBaseline ) manifest_destroy(db, p->pBaseline);
    memset(p, 0, sizeof(*p));
    geomvcs_free(p);
  }
}
...
*/
void manifest_cache_insert(struct vcs_db *db, Manifest *p){
  while( p ){
    int i;
    ...
    if( i>=MX_MANIFEST_CACHE ){
      ...
      manifest_destroy(db, manifestCache.apManifest[oldest]);
      i = oldest;
    }
...
*/
void manifest_cache_clear(struct vcs_db *db){
  int i;
  for(i=0; i<MX_MANIFEST_CACHE; i++){
    if( manifestCache.apManifest[i] ){
      manifest_destroy(db, manifestCache.apManifest[i]);
    }
  }
...
*/
static int verify_z_card(struct vcs_db *db, const char *z, int n){
  if( n<35 ) return 0;
  if( z[n-35]!='Z' || z[n-34]!=' ' ) return 0;
  md5sum_init();
  md5sum_step_text(z, n-35);
  if( memcmp(&z[n-33], md5sum_finish(db, 0), 32)==0 ){
    return 1;
  }else{
    return 2;
  }
...
*/
static Manifest *manifest_parse(struct vcs_db *db, Blob *pContent, int rid){
  Manifest *p;
  int seenZ = 0;
  int i, lineNo=0;
  ManifestText x;
    ...
    switch( cType ){
      ...
      */
      case 'D': {
        ...
        p->rDate = db_double(db, 0.0, "SELECT julianday(%Q)", next_token(&x,0));
        if( p->rDate<=0.0 ) goto manifest_syntax_error;
        break;
      }
      ...
      */
      case 'E': {
        ...
        p->rEventDate = db_double(db, 0.0,"SELECT julianday(%Q)", next_token(&x,0));
        if( p->rEventDate<=0.0 ) goto manifest_syntax_error;
      ...
      */
      case 'F': {
        ...
        if( p->nFile>=p->nFileAlloc ){
          ...
          p->aFile = geomvcs_realloc(db, p->aFile, 
        ...
        if( i>0 && geomvcs_strcmp(p->aFile[i-1].zName, zName)>=0 ){
          goto manifest_syntax_error;
        }
        break;
      }
      ...
      */
      case 'J': {
        ...
        if( p->nField>=p->nFieldAlloc ){
          ...
          p->aField = geomvcs_realloc(db, p->aField,
                               p->nFieldAlloc*sizeof(p->aField[0]) );
        }
        i = p->nField++;
        p->aField[i].zName = zName;
        p->aField[i].zValue = zValue;
        if( i>0 && geomvcs_strcmp(p->aField[i-1].zName, zName)>=0 ){
          goto manifest_syntax_error;
        }
        break;
      }
      ...
      */
      case 'M': {
        ...
        if( p->nCChild>=p->nCChildAlloc ){
          ...
          p->azCChild = geomvcs_realloc(db, p->azCChild
                                 , p->nCChildAlloc*sizeof(p->azCChild[0]) );
        }
        i = p->nCChild++;
        p->azCChild[i] = zUuid;
        if( i>0 && geomvcs_strcmp(p->azCChild[i-1], zUuid)>=0 ){
          goto manifest_syntax_error;
        }
        break;
      }
      ...
      case 'P': {
        while( (zUuid = next_token(&x, &sz))!=0 ){
          ...
          if( p->nParent>=p->nParentAlloc ){
            ...
            p->azParent = geomvcs_realloc(db, p->azParent,
      ...
      */
      case 'Q': {
        ...
        n = p->nCherrypick;
        p->nCherrypick++;
        p->aCherrypick = geomvcs_realloc(db, p->aCherrypick,
      ...
      */
      case 'T': {
        ...
        if( p->nTag>=p->nTagAlloc ){
          ...
          p->aTag = geomvcs_realloc(db, p->aTag, p->nTagAlloc*sizeof(p->aTag[0]) );
        }
        i = p->nTag++;
        p->aTag[i].zName = zName;
        p->aTag[i].zUuid = zUuid;
        p->aTag[i].zValue = zValue;
        if( i>0 && geomvcs_strcmp(p->aTag[i-1].zName, zName)>=0 ){
          goto manifest_syntax_error;
        }
        break;
      }
...
*/
Manifest *manifest_get(struct vcs_db *db, int rid, int cfType){
  Blob content;
  Manifest *p;
  if( !rid ) return 0;
...
*/
Manifest *manifest_get_by_name(struct vcs_db *db, const char *zName, int *pRid){
  int rid;
  Manifest *p;

  rid = name_to_rid(zName);
  if( !is_a_version(rid) ){
    geomvcs_fatal(db, "no such checkin: %s", zName);
  }
  if( pRid ) *pRid = rid;
  p = manifest_get(db, rid, CFTYPE_MANIFEST);
  if( p==0 ){
    geomvcs_fatal(db, "cannot parse manifest for checkin: %s", zName);
  }
  return p;
}
...
*/
static int fetch_baseline(struct vcs_db *db, Manifest *p, int throwError){
  if( p->zBaseline!=0 && p->pBaseline==0 ){
    int rid = uuid_to_rid(p->zBaseline, 1);
    p->pBaseline = manifest_get(db, rid, CFTYPE_MANIFEST);
    if( p->pBaseline==0 ){
      if( !throwError ){
        db_multi_exec(db,
      ...
      geomvcs_fatal(db, "cannot access baseline manifest %S", p->zBaseline);
    }
  }
  return 0;
}
    ...
    while(1){
      if( pB->iFile>=pB->nFile ){
        ...
        ** entry from the baseline. */
        ...
        break;
      }else if( (cmp = geomvcs_strcmp(pB->aFile[pB->iFile].zName,
...
*/
static int filename_to_fnid(struct vcs_db *db, const char *zFilename){
  static Stmt q1, s1;
  int fnid;
  db_static_prepare(db, &q1, "SELECT fnid FROM filename WHERE name=:fn");
  db_bind_text(db, &q1, ":fn", zFilename);
  fnid = 0;
  ...
  db_reset(db, &q1);
  if( fnid==0 ){
    db_static_prepare(db, &s1, "INSERT INTO filename(name) VALUES(:fn)");
    db_bind_text(db, &s1, ":fn", zFilename);
    db_exec(db, &s1);
    fnid = db_last_insert_rowid(db);
  }
  return fnid;
}
...
static void add_one_mlink(
  ...
  fnid = filename_to_fnid(db, zFilename);
  if( zPrior==0 ){
    pfnid = 0;
  }else{
    pfnid = filename_to_fnid(db, zPrior);
  }
  ...
  db_bind_int(db, &s1, ":m", mid);
  db_bind_int(db, &s1, ":p", pid);
  db_bind_int(db, &s1, ":f", fid);
  db_bind_int(db, &s1, ":n", fnid);
  db_bind_int(db, &s1, ":pfn", pfnid);
...
static ManifestFile *manifest_file_seek_base(Manifest *p, const char *zName){
  ...
  if( p->iFile>=lwr && p->iFile<upr ){
    c = geomvcs_strcmp(p->aFile[p->iFile+1].zName, zName);
    if( c==0 ){
  ...
  while( lwr<=upr ){
    i = (lwr+upr)/2;
    c = geomvcs_strcmp(p->aFile[i].zName, zName);
    if( c<0 ){
      lwr = i+1;
    }else if( c>0 ){
      upr = i-1;
...
*/
ManifestFile *manifest_file_seek(struct vcs_db *db, Manifest *p, const char *zName){
  ManifestFile *pFile;
...
*/
static void add_mlink(struct vcs_db *db, int pid, Manifest *pParent, int cid, Manifest *pChild){
  Blob otherContent;
  int otherRid;
  int i, rc;
  ...
  ** doing no work.
  */
  db_static_prepare(db, &eq, "SELECT 1 FROM mlink WHERE mid=:mid");
  db_bind_int(db, &eq, ":mid", cid);
  rc = db_step(&eq);
  db_reset(db, &eq);
  ...
  */
  ...
  if( fetch_baseline(db, pParent, 0) || fetch_baseline(db, pChild, 0) ){
  ...
  */  
  for(i=0, pChildFile=pChild->aFile; i<pChild->nFile; i++, pChildFile++){
    ...
    if( pChildFile->zPrior ){
       pParentFile = manifest_file_seek(db, pParent, pChildFile->zPrior);
       if( pParentFile ){
         /* File with name change */
         add_one_mlink(db, cid, pParentFile->zUuid, pChildFile->zUuid,
         ...
         ** Treat this like a new file. */
         add_one_mlink(db, cid, 0, pChildFile->zUuid, pChildFile->zName, 0,
                       isPublic, mperm);
       }
    }else{
       pParentFile = manifest_file_seek(db, pParent, pChildFile->zName);
       if( pParentFile==0 ){
         if( pChildFile->zUuid ){
           /* A new file */
           add_one_mlink(db, cid, 0, pChildFile->zUuid, pChildFile->zName, 0,
       ...
       }else if( geomvcs_strcmp(pChildFile->zUuid, pParentFile->zUuid)!=0
  ...
  if( pParent->zBaseline && pChild->zBaseline ){
    ...
    ** and show such files as being added in the child. */
    for(i=0, pParentFile=pParent->aFile; i<pParent->nFile; i++, pParentFile++){
      ...
      pChildFile = manifest_file_seek(db, pChild, pParentFile->zName);
      if( pChildFile ){
        add_one_mlink(db, cid, 0, pChildFile->zUuid, pChildFile->zName, 0,
    ...
    ** has having been deleted. */
    manifest_file_rewind(db, pParent);
    while( (pParentFile = manifest_file_next(pParent,0))!=0 ){
      pChildFile = manifest_file_seek(db, pChild, pParentFile->zName);
      if( pChildFile==0 ){
        add_one_mlink(db, cid, pParentFile->zUuid, 0, pParentFile->zName, 0, 
                      isPublic, 0);
      }
    }
  }
  manifest_cache_insert(db, *ppOther);
}
...
*/
void manifest_crosslink_begin(struct vcs_db *db){
...
*/
void manifest_crosslink_end(struct vcs_db *db){
  Stmt q, u;
  int i;
  assert( manifest_crosslink_busy==1 );
  db_prepare(db, &q, "SELECT uuid FROM pending_tkt");
  while( db_step(&q)==SQLITE_ROW ){
    const char *zUuid = db_column_text(&q, 0);
    ticket_rebuild_entry(zUuid);
  }
  db_finalize(db, &q);
  db_multi_exec(db, "DROP TABLE pending_tkt");
  ...
  */
  ...
  db_bind_double(db, &q, ":incr", AGE_ADJUST_INCREMENT);
  db_bind_double(db, &q, ":window", AGE_FUDGE_WINDOW);
  db_prepare(db, &u,
  ...
  for(i=0; i<30; i++){
    db_step(&q);
    db_reset(db, &q);
    if( sqlite3_changes(db->db)==0 ) break;
    db_step(&u);
    db_reset(db, &u);
  }
  db_finalize(db, &q);
...
void manifest_ticket_event(
  ...
  if( once ){
    once = 0;
    zTitleExpr = db_get(db, "ticket-title-expr", "title");
    zStatusColumn = db_get(db, "ticket-status-column", "status");
  }
  zTitle = db_text(db, "unknown", 
    "SELECT %s FROM ticket WHERE tkt_uuid='%s'",
    zTitleExpr, pManifest->zTicketUuid
  );
  if( !isNew ){
    for(i=0; i<pManifest->nField; i++){
      if( geomvcs_strcmp(pManifest->aField[i].zName, zStatusColumn)==0 ){
        zNewStatus = pManifest->aField[i].zValue;
      }
    }
    if( zNewStatus ){
      blob_appendf(db, &comment, "%h ticket [%.10s]: <i>%s</i>",
         zNewStatus, pManifest->zTicketUuid, zTitle
      );
      if( pManifest->nField>1 ){
        blob_appendf(db, &comment, " plus %d other change%s",
          pManifest->nField-1, pManifest->nField==2 ? "" : "s");
      }
      blob_appendf(db, &brief, "%h ticket [%.10s].",
                   zNewStatus, pManifest->zTicketUuid);
    }else{
      zNewStatus = db_text(db, "unknown", 
         "SELECT %s FROM ticket WHERE tkt_uuid='%s'",
         zStatusColumn, pManifest->zTicketUuid
      );
      blob_appendf(db, &comment, "Ticket [%.10s] <i>%s</i> status still %h with "
           "%d other change%s",
           pManifest->zTicketUuid, zTitle, zNewStatus, pManifest->nField,
           pManifest->nField==1 ? "" : "s"
      );
      free(zNewStatus);
      blob_appendf(db, &brief, "Ticket [%.10s]: %d change%s",
           pManifest->zTicketUuid, pManifest->nField,
           pManifest->nField==1 ? "" : "s"
      );
    }
  }else{
    blob_appendf(db, &comment, "New ticket [%.10s] <i>%h</i>.",
      pManifest->zTicketUuid, zTitle
    );
    blob_appendf(db, &brief, "New ticket [%.10s].", pManifest->zTicketUuid);
  }
  free(zTitle);
  db_multi_exec(db,
    ...
    blob_str(db, &comment), blob_str(db, &brief)
  );
  blob_reset(db, &comment);
  blob_reset(db, &brief);
}
...
int manifest_crosslink(struct vcs_db *db, int rid, Blob *pContent){
  ...
  if( (p = manifest_cache_find(rid))!=0 ){
    blob_reset(db, pContent);
  }else if( (p = manifest_parse(db, pContent, rid))==0 ){
    assert( blob_is_reset(pContent) || pContent==0 );
    return 0;
  }
  if( db->xlinkClusterOnly && p->type!=CFTYPE_CLUSTER ){
    manifest_destroy(db, p);
    assert( blob_is_reset(pContent) );
    return 0;
  }
  if( p->type==CFTYPE_MANIFEST && fetch_baseline(db, p, 0) ){
    manifest_destroy(db, p);
    assert( blob_is_reset(pContent) );
    return 0;
  }
  ...
  if( p->type==CFTYPE_MANIFEST ){
    if( !db_exists(db, "SELECT 1 FROM mlink WHERE mid=%d", rid) ){
      char *zCom;
      for(i=0; i<p->nParent; i++){
        int pid = uuid_to_rid(p->azParent[i], 1);
        db_multi_exec(db, "INSERT OR IGNORE INTO plink(pid, cid, isprim, mtime)"
                      "VALUES(%d, %d, %d, %.17g)", pid, rid, i==0, p->rDate);
        if( i==0 ){
          add_mlink(db, pid, 0, rid, p);
          parentid = pid;
        }
      }
      db_prepare(db, &q, "SELECT cid FROM plink WHERE pid=%d AND isprim", rid);
      while( db_step(&q)==SQLITE_ROW ){
        int cid = db_column_int(&q, 0);
        add_mlink(db, rid, p, cid, 0);
      }
      db_finalize(db, &q);
      if( p->nParent==0 ){
      ...
      zCom = db_text(db, 0, "SELECT coalesce(ecomment, comment) FROM event"
      ...
      */
      if( p->zBaseline!=0 ){
        static int once = 0;
        if( !once ){
          db_set_int(db, "seen-delta-manifest", 1, 0);
          once = 0;
        }
      }
    }
  ...
  if( p->type==CFTYPE_CLUSTER ){
    ...
    db_static_prepare(db, &del1, "DELETE FROM unclustered WHERE rid=:rid");
    for(i=0; i<p->nCChild; i++){
      int mid;
      mid = uuid_to_rid(p->azCChild[i], 1);
      if( mid>0 ){
        db_bind_int(db, &del1, ":rid", mid);
        db_step(&del1);
        db_reset(db, &del1);
      }
    }
    ...
    for(i=0; i<p->nTag; i++){
      ...
      if( tid ){
        switch( p->aTag[i].zName[0] ){
          ...
          default:
            geomvcs_fatal(db, "unknown tag type in manifest: %s", p->aTag);
            return 0;
        }
  ...
  if( p->type==CFTYPE_WIKI ){
    char *zTag = mprintf(db, "wiki-%s", p->zWikiTitle);
    int tagid = tag_findid(zTag, 1);
    int prior;
    char *zComment;
    int nWiki;
    char zLength[40];
    while( geomvcs_isspace(p->zWiki[0]) ) p->zWiki++;
    ...
    if( nWiki>0 ){
      ...
      zComment = mprintf(db, "Deleted wiki page [%h]", p->zWikiTitle);
    }
    db_multi_exec(db,
      "REPLACE INTO event(type,mtime,objid,user,comment,"
  ...
  if( p->type==CFTYPE_EVENT ){
    char *zTag = mprintf(db, "event-%s", p->zEventId);
    int tagid = tag_findid(zTag, 1);
  ...
  if( p->type==CFTYPE_TICKET ){
    ...
    assert( manifest_crosslink_busy==1 );
    zTag = mprintf(db, "tkt-%s", p->zTicketUuid);
    tag_insert(zTag, 1, 0, rid, p->rDate, rid);
    free(zTag);
    db_multi_exec(db, "INSERT OR IGNORE INTO pending_tkt VALUES(%Q)",
                  p->zTicketUuid);
  }
    ...
    if( strlen(p->zAttachTarget)!=UUID_SIZE
     || !validate16(p->zAttachTarget, UUID_SIZE) 
      ...
      if( p->zAttachSrc && p->zAttachSrc[0] ){
        zComment = mprintf(db, "Add attachment \"%h\" to wiki page [%h]",
             p->zAttachName, p->zAttachTarget);
      }else{
        zComment = mprintf(db, "Delete attachment \"%h\" from wiki page [%h]",
             p->zAttachName, p->zAttachTarget);
      }
      db_multi_exec(db,
        "REPLACE INTO event(type,mtime,objid,user,comment)"
      ...
      char *zComment;
      if( p->zAttachSrc && p->zAttachSrc[0] ){
        zComment = mprintf(db, "Add attachment \"%h\" to ticket [%.10s]",
             p->zAttachName, p->zAttachTarget);
      }else{
        zComment = mprintf(db, "Delete attachment \"%h\" from ticket [%.10s]",
             p->zAttachName, p->zAttachTarget);
      }
      db_multi_exec(db,
        "REPLACE INTO event(type,mtime,objid,user,comment)"
