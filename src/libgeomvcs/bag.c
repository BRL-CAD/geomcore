...
/* Forward declarations for bag.c */
...
*/
static void bag_resize(struct vcs_db *db, Bag *p, int newSize){
  int i;
  Bag old;
  ...
  p->a = geomvcs_malloc(db, sizeof(p->a[0])*newSize );
  p->sz = newSize;
