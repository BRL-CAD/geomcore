...
/* Forward declarations for diff.c */
...
** too long.
*/
static DLine *break_into_lines(struct vcs_db *db, const char *z, int n, int *pnLine, int ignoreWS){
  int nLine, i, j, k, x;
  unsigned int h, h2;
  DLine *a;
  ...
  */
  ...
  a = geomvcs_malloc(db, nLine*sizeof(a[0]) );
  ...
  for(i=0; i<nLine; i++){
    ...
    for(j=0; z[j] && z[j]!='\n'; j++){}
    k = j;
    while( ignoreWS && k>0 && geomvcs_isspace(z[k-1]) ){ k--; }
    for(h=0, x=0; x<k; x++){
      h = h ^ (h<<2) ^ z[x];
    }
...
*/
static void appendDiffLine(struct vcs_db *db, Blob *pOut, char *zPrefix, DLine *pLine){
  blob_append(db, pOut, zPrefix, 1);
  blob_append(db, pOut, pLine->z, pLine->h & LENGTH_MASK);
  blob_append(db, pOut, "\n", 1);
}
...
*/
static void expandEdit(struct vcs_db *db, DContext *p, int nEdit){
  p->aEdit = geomvcs_realloc(db, p->aEdit, nEdit*sizeof(int));
  p->nEditAlloc = nEdit;
}
...
*/
static void appendTriple(struct vcs_db *db, DContext *p, int nCopy, int nDel, int nIns){
  /* printf("APPEND %d/%d/%d\n", nCopy, nDel, nIns); */
  if( p->nEdit>=3 ){
...
*/
static void contextDiff(struct vcs_db *db, DContext *p, Blob *pOut, int nContext){
...
*/
static void diff_step(struct vcs_db *db, DContext *p, int iS1, int iE1, int iS2, int iE2){
  int iSX, iEX, iSY, iEY;

  if( iE1<=iS1 ){
    /* The first segment is empty */
    if( iE2>iS2 ){
      appendTriple(db, p, 0, 0, iE2-iS2);
    }
    return;
  }
  if( iE2<=iS2 ){
    /* The second segment is empty */
    appendTriple(db, p, 0, iE1-iS1, 0);
    return;
  }
  ...
  if( iEX>iSX ){
    ...
    ** Recursively diff either side of the matching segment */
    diff_step(db, p, iS1, iSX, iS2, iSY);
    if( iEX>iSX ){
      appendTriple(db, p, iEX - iSX, 0, 0);
    }
    diff_step(db, p, iEX, iE1, iEY, iE2);
  }else{
    /* The two segments have nothing in common.  Delete the first then
    ** insert the second. */
    appendTriple(db, p, 0, iE1-iS1, iE2-iS2);
  }
}
...
*/
static void diff_all(struct vcs_db *db, DContext *p){
  int mnE, iS, iE1, iE2;
  ...
  /* do the difference */
  if( iS>0 ){
    appendTriple(db, p, iS, 0, 0);
  }
  diff_step(db, p, iS, iE1, iS, iE2);
  if( iE1<p->nFrom ){
    appendTriple(db, p, p->nFrom - iE1, 0, 0);
  }

  /* Terminate the COPY/DELETE/INSERT triples with three zeros */
  expandEdit(db, p, p->nEdit+3);
  if( p->aEdit ){
    p->aEdit[p->nEdit++] = 0;
    p->aEdit[p->nEdit++] = 0;
    p->aEdit[p->nEdit++] = 0;
...
int *text_diff(
  ...
  memset(&c, 0, sizeof(c));
  c.aFrom = break_into_lines(db, blob_str(db, pA_Blob), blob_size(pA_Blob),
                             &c.nFrom, ignoreEolWs);
  c.aTo = break_into_lines(db, blob_str(db, pB_Blob), blob_size(pB_Blob),
                           &c.nTo, ignoreEolWs);
  if( c.aFrom==0 || c.aTo==0 ){
    free(c.aFrom);
    free(c.aTo);
    if( pOut ){
      blob_appendf(db, pOut, "cannot compute difference between binary files\n");
    }
    return 0;
  }
  ...
  if( pOut ){
    ...
    contextDiff(db, &c, pOut, nContext);
    free(c.aFrom);
    free(c.aTo);
    free(c.aEdit);
    return 0;
...
*/
static int annotation_start(struct vcs_db *db, Annotator *p, Blob *pInput){
  int i;

  memset(p, 0, sizeof(*p));
  p->c.aTo = break_into_lines(db, blob_str(db, pInput), blob_size(pInput),&p->c.nTo,1);
  if( p->c.aTo==0 ){
    return 1;
  }
  p->aOrig = geomvcs_malloc(db, sizeof(p->aOrig[0])*p->c.nTo );
  for(i=0; i<p->c.nTo; i++){
...
*/
static int annotation_step(struct vcs_db *db, Annotator *p, Blob *pParent, char *zPName){
  int i, j;
  int lnTo;

  /* Prepare the parent file to be diffed */
  p->c.aFrom = break_into_lines(db, blob_str(db, pParent), blob_size(pParent),
                                &p->c.nFrom, 1);
  if( p->c.aFrom==0 ){
    return 1;
  }
...
*/
static void annotate_file(struct vcs_db *db, Annotator *p, int fnid, int mid, int webLabel){
  ...
  rid = db_int(db, 0, "SELECT fid FROM mlink WHERE mid=%d AND fnid=%d",mid,fnid);
  if( rid==0 ){
  ...
  compute_ancestors(db, mid, 1000000000);
  annotation_start(db, p, &toAnnotate);
  ...
  while( db_step(&q)==SQLITE_ROW ){
    ...
    if( webLabel ){
      zLabel = mprintf(db, "<a href='%s/info/%s'>%.10s</a> %s %9.9s", 
                       db->zTop, zUuid, zUuid, zDate, zUser);
    }else{
