...
*/
char *sha1sum_finish(struct vcs_db *db, Blob *pOut){
...
*/
int sha1sum_file(struct vcs_db *db, const char *zFilename, Blob *pCksum){
  FILE *in;
  SHA1Context ctx;
  unsigned char zResult[20];
  char zBuf[10240];
...
** Return the number of errors.
*/
int sha1sum_blob(struct vcs_db *db, const Blob *pIn, Blob *pCksum){
  SHA1Context ctx;
  unsigned char zResult[20];
...
*/
char *sha1sum(struct vcs_db *db, const char *zIn){
  SHA1Context ctx;
  unsigned char zResult[20];
  char zDigest[41];
...
*/
char *sha1_shared_secret(struct vcs_db *db, const char *zPw, const char *zLogin){
  ...
  SHA1Init(&ctx);
  if( zProjectId==0 ){
    zProjectId = db_get(db, "project-code", 0);
...
*/
void sha1sum_test(struct vcs_db *db){
  int i;
  Blob in;
  Blob cksum;
  ...
  for(i=2; i<db->argc; i++){
    ...
    if( db->argv[i][0]=='-' && db->argv[i][1]==0 ){
      blob_read_from_channel(db, &in, stdin, -1);
      sha1sum_blob(db, &in, &cksum);
    }else{
      sha1sum_file(db, db->argv[i], &cksum);
    }
    printf("%s  %s\n", blob_str(db, &cksum), db->argv[i]);
    blob_reset(db, &cksum);
  }
}
