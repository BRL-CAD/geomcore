...
/****** THIS BELONGS IN A CLIENT PROGRAM, NOT THE LIBRARY *********/

/*
** Remove n elements from g.argv beginning with the i-th element.
*/
void remove_from_argv(struct vcs_db *db, int i, int n){
  int j;
  for(j=i+n; j<db->argc; i++, j++){
    db->argv[i] = db->argv[j];
  }
  db->argc = i;
}
...
const char *find_option(struct vcs_db *db, const char *zLong, const char *zShort, int hasArg){
  ...
  for(i=1; i<db->argc; i++){
    char *z;
    if (i+hasArg >= db->argc) break;
    z = db->argv[i];
    if( z[0]!='-' ) continue;
    z++;
    if( z[0]=='-' ){
      if( z[1]==0 ){
        remove_from_argv(db, i, 1);
        break;
      }
      z++;
    }
    if( strncmp(z,zLong,nLong)==0 ){
      if( hasArg && z[nLong]=='=' ){
        ...
        zReturn = db->argv[i+hasArg];
        remove_from_argv(db, i, 1+hasArg);
        break;
      }
    }else if( geomvcs_strcmp(z,zShort)==0 ){
      zReturn = db->argv[i+hasArg];
      remove_from_argv(db, i, 1+hasArg);
      break;
    }
