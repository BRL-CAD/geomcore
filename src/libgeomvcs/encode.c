...
char *htmlize(struct vcs_db *db, const char *zIn, int n){
  ...
  i = 0;
  zOut = geomvcs_malloc(db, count+1);
  while( n-->0 && (c = *zIn)!=0 ){
    switch( c ){
      case '<':   
        zOut[i++] = '&';
...
*/
static char *EncodeHttp(struct vcs_db *db, const char *zIn, int n, int encodeSlash){
  int c;
  int i = 0;
  int count = 0;
  char *zOut;
...
*/
char *encode64(struct vcs_db *db, const char *zData, int nData){
  char *z64;
  int i, n;
  ...
  z64 = geomvcs_malloc(db, (nData*4)/3 + 8 );
...
*/
char *decode64(struct vcs_db *db, const char *z64, int *pnByte){
  char *zData;
  int n64;
  int i, j;
  int a, b, c, d;
  ...
  zData = geomvcs_malloc(db, (n64*3)/4 + 4 );
...
*/
char *obscure(struct vcs_db *db, const char *zIn){
  int n, i;
  unsigned char salt;
  char *zOut;
...
*/
char *unobscure(struct vcs_db *db, const char *zIn){
  int n, i;
  unsigned char salt;
  char *zOut;
