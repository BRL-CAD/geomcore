...
int
gvm_new_model(struct gvm_info *repo_info, const char *model_name) {
    int ret = 0;
    apr_pool_t *subpool;
...
int
gvm_get_model(struct gvm_info *repo_info, const char *model_name, size_t ver_num) {
    struct geomsvn_info *internal = NULL;
    internal = (struct geomsvn_info *)repo_info->internal;
...
int
_add_comb_entries(apr_hash_t *objects, apr_hash_t *tmp, struct rt_comb_internal *comb, const union tree *treep) {
    int count = 0;
    const union tree *tp;
    if (comb) {
	tp = comb->tree;
