...
GVM_EXPORT extern int
gvm_init_repo(struct gvm_info *repo_info,
		 const char *repo_path);

/* Open a repository. Note that this may change to add some
 * sort of authentication paramaters. */
GVM_EXPORT extern int
gvm_open_repo(struct gvm_info *repo_info,
		 const char *repo_path);
 ...
 * latest revision */
GVM_EXPORT extern int
gvm_object_in_model(struct gvm_info *repo_info,
...
GVM_EXPORT extern struct bu_external *
gvm_get_extern_obj(struct gvm_info *repo_info,
		 const char *model_name,
		 const char *obj_name,
		 size_t ver_num);
 ...
 * the repository */
GVM_EXPORT extern struct repository_objects *
gvm_get_repo_obj(struct gvm_info *repo_info,
		 const char *model_name,
		 const char *obj_name,
		 size_t ver_num);
 ...
 * they differ */
GVM_EXPORT extern int
gvm_diff(struct gvm_info *repo_info,
 ...
 * objects list */
GVM_EXPORT extern int
gvm_add_to_list(struct gvm_info *repo_info,
		struct repository_objects *obj);

/* Commit objects in repo_info's objects list to
 * repository. */
GVM_EXPORT extern int
gvm_commit_objs(struct gvm_info *repo_info);
...
GVM_EXPORT extern int
gvm_new_model(struct gvm_info *repo_info,
		const char *model_name);
...
GVM_EXPORT extern int
gvm_get_model(struct gvm_info *repo_info,
		 const char *model_name,
		 size_t ver_num);
...
GVM_EXPORT extern int
gvm_get_objs(struct gvm_info *repo_info,
		 const char *model_name,
		 const char *obj_name,
		 size_t ver_num,
		 int recursive);
...
GVM_EXPORT extern int
gvm_import_g_file(struct gvm_info *repo_info,
		 const char *g_file);

/* Update an existing model repository using a .g file. */
GVM_EXPORT extern int
gvm_commit_g_file(struct gvm_info *repo_info,
		 const char *model_name,
		 const char *g_file);
 ...
 * is LATEST_VERSION use latest revision */
GVM_EXPORT extern int
gvm_export_g_file(struct gvm_info *repo_info,
		 const char *model_name,
		 const char *g_file,
		 size_t ver_num);
...
GVM_EXPORT extern int
gvm_export_list(struct gvm_info *repo_info,
		 const char *model_name,
		 const char *g_file);
 ...
 * object. */
GVM_EXPORT extern int
gvm_export_object(struct gvm_info *repo_info,
		 const char *model_name,
		 const char *obj_name,
		 const char *g_file,
		 size_t ver_num,
